...
public class FireBurstSphere extends ActiveAbility {
	...
	public FireBurstSphere(Player player, IAbility parent) {
		...
		if (burst.isCharged()) {
			...
			List<Block> safeblocks = BlockTools.getBlocksAroundPoint(player.getLocation(), 2);
			...
			for (double theta = 0; theta <= 180; theta += this.deltheta) {
				...
				for (double phi = 0; phi < 360; phi += dphi) {
					...
					new FireBlast(player, this, location, direction.normalize(), this.damage, safeblocks);
				}
			}
			burst.remove();
		}
