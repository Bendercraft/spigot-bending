...
/**
 * Class that manages combo points for a chiblocker (There is no check about being a chiblocker or
 * not)
 *
 * @author Noko
 */
public class ComboPoints {
	 ...
	 * @param target
	 *        The target that the player has hit to get the combo point. Know that if the target is
	 *        different from the previous target, combo points will be reset
	 * @return <code>true</code> if the combo point has been added (even if already at maximum)
	 *         <code>false</code> if no target or if too soon after the previous combo point
	 *         addition
	 */
	 ...
	 * @return
	 *         <code>0</code> if the player is null or never got point before
	 *         <code> a number between 0 and 5 included</code> according to the player combo amount
	 */
	 ...
	 *         <code>false</code> if no player, if the amount is less than 0 or if player has less
	 *         combo point that he wants to use
	 */
