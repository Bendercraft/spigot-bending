...
public class EarthBlast extends ActiveAbility {
	...
	private void cancelPrevious(Block b) {
		...
		for (IAbility ab : AbilityManager.getManager().getInstances(getAbilityType()).values()) {
			...
			if ((blast.sourceblock.equals(b) || (blast.player == this.player)) && (blast.state == AbilityState.Prepared)) {
				blast.cancel();
				toRemove.add(blast);
			}
		}
	...
	public boolean progress() {
		...
		if (this.player.isDead() || !this.player.isOnline()
				|| !EntityTools.canBend(this.player, Abilities.EarthBlast)) {
			breakBlock();
			return false;
		}
