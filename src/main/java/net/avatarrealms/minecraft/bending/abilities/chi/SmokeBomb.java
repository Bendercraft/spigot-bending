...
public class SmokeBomb {
	...
	private static Type fwType = Type.BURST;
	private static Color fwColor = Color.BLACK;
	...
	public SmokeBomb(Player player) {
		...
		if (!bPlayer.isOnCooldown(Abilities.SmokeBomb)) {
			...
			Firework fw = (Firework) bomber.getWorld().spawnEntity(origin,EntityType.FIREWORK);
			...
			for (Block block : blocks) {
				locs.add(block.getLocation().clone().add(0,1,0));
			}
	...
	public void progress() {
		...
		List<Entity> entitiesAround = EntityTools.getEntitiesAroundPoint(origin,radius);
		...
		blindnessTarget = new PotionEffect(PotionEffectType.BLINDNESS, ticksRemaining,2);
